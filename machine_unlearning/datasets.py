...
from .utils import resolve_classes

class FacesDataset(Dataset):
    def __init__(self, path, data, transform, age_ranges, age_from_path=lambda x: int(x.split('_')[0])):
        self.path = path
        ...
        for i in range(len(age_ranges) - 1):            
            ...
            self.data += [d for d in data if from_ <= age_from_path(d) < to_]
            self.targets += [i for d in data if from_ <= age_from_path(d) < to_]

        self.classes = sorted(np.unique(self.targets))
...
class AgeDB(Dataset):
    def __init__(self, path, data, transform, age_ranges):
        self.path = path
        self.data, self.targets = [], []
        for i in range(len(age_ranges) - 1):            
            from_ = age_ranges[i]
            ...
            self.data += [d[0] for d in data if from_ <= int(d[1]) < to_]
            self.targets += [i for d in data if from_ <= int(d[1]) < to_]
